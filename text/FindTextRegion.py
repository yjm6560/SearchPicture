...
TODO
*조정이 필요한 parameter들
adaptiveTreshold() : block_size, subtract_val
...
class FindTextRegion:
    ...
    def adaptiveThreshold(self, gray_image, block_size = 5, subtract_val = 15):
        #block_size : 픽셀에 적용할 threshold 값을 계산하기 위한 블럭 크기. 적용될 픽셀이 블럭의 중심이 됨. 따라서 홀수만 가능
        #subtract_val : 보정 상수
        adaptive_gaussian_image = cv2.adaptiveThreshold(gray_image, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, block_size, subtract_val)
        return adaptive_gaussian_image

    def morphClose(self, adaptive_gaussian_image, width = 5, height = 5, iter = 1):
        ...
        #2번 방법
        dilation = cv2.dilate(adaptive_gaussian_image, kernel, iterations=iter)
        erosion = cv2.erode(adaptive_gaussian_image, kernel, iterations=iter)
